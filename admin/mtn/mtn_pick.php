...
if(!empty($op)) {
	...
	switch ($op) {
		case 'add':
			...
			$mtnp_status = GetParam('mtnp_status');
			
			if(empty($emm_sn)||empty($jsuser_sn)||$mtnp_status=='') {
				$ret_msg = "*為必填！";
			} else {
				$now = time();
				$conn = getDB();
				$sql = "INSERT INTO mtn_pick (mtnp_add_date, mtnp_mod_date, mtnp_status, emm_sn, jsuser_sn) " . 
					"VALUES ('{$now}', '{$now}', '{$mtnp_status}', '{$emm_sn}', '{$jsuser_sn}');";
				if($conn->query($sql)) {
					$ret_msg = "新增成功！";
				} else {
					$ret_msg = "新增失敗！";
		...
		case 'get':
			$mtnp_sn = GetParam('mtnp_sn');
			$ret_data = array();
			if(!empty($mtnp_sn)){
				$ret_code = 1;
				$ret_data = getConstLogBySn($mtnp_sn);
			} else {
				$ret_code = 0;
			}
		...
		case 'upd':
			...
			if(empty($mtnp_sn)||$mtnp_status==''||empty($emm_sn)||empty($jsuser_sn)) {
				$ret_msg = "*為必填！";
			} else {
				$now = time();
				$conn = getDB();
				$sql = "UPDATE mtn_pick SET mtnp_status='{$mtnp_status}', emm_sn='{$emm_sn}', jsuser_sn='{$jsuser_sn}', mtnp_mod_date='{$now}' WHERE mtnp_sn='{$mtnp_sn}'";
				if($conn->query($sql)) {
					$ret_msg = "修改完成！";
				} else {
					$ret_msg = "修改失敗！";
		...
		case 'del':
			...
			if(empty($mtnp_sn)){
				...
				$now = time();
				$conn = getDB();
				$sql = "UPDATE mtn_pick SET mtnp_status=-1, mtnp_mod_date='{$now}' WHERE mtnp_sn='{$mtnp_sn}'";
				if($conn->query($sql)) {
					$ret_msg = "刪除完成！";
				} else {
					$ret_msg = "刪除失敗！";
		...
		case 'show_photo':
			...
			if(!empty($mtnp_sn)){
				$ret_code = 1;
				$ret_data = getPhoto('mtn_pick', $mtnp_sn);
			} else {
				$ret_code = 0;
			}
			break;
		...
		case 'add_photo':
			...
			if(!empty($mtnp_sn)){
				for($i=1; $i<=5; $i++) {
					uploaded_photo('mtn_pick', $mtnp_sn, 'filetoupload'.$i);
				}
	...
	// init define
	define('WEB_PAGE_TITLE', "領料紀錄表單管理");
	define('ADD_MODAT_TITLE', "新增表單");
	define('UPDATE_MODAT_TITLE', "修改表單");
	define('PAGE_FILE_NAME', "mtn_pick.php");
	...
	if(($start = GetParam('start',""))) {
		...
		$search_where[] = "a.mtnp_add_date>={$start_c}";
		$search_query_string['start'] = $start;
	}

	if(($end = GetParam('end',""))) {
		$end_c = str2time($end.":59");
		$search_where[] = "a.mtnp_add_date<={$end_c}";
		$search_query_string['end'] = $end;
	}
	
	if(($mtnp_status = GetParam('mtnp_status', -1))>=0) {
		$search_where[] = "a.mtnp_status='{$mtnp_status}'";
		$search_query_string['mtnp_status'] = $mtnp_status;
	}
    ...
    <head>
		...
		<script type="text/javascript">
			$(document).ready(function() {
				<?php 
					//	init search parm
					print "$('#search [name=mtnp_status] option[value={$mtnp_status}]').prop('selected','selected');";
				?>
				...
				$('button.upd').on('click', function(){
					...
					$.ajax({
					    ...
					    type: 'post',
				        dataType: 'json',
					    data: {op:"get", mtnp_sn:$(this).data('mtnp_sn')},
		                ...
		                success: function(ret) {
			                ...
			                if(ret.code==1) {
								...
								$('#upd_form input[name=mtnp_sn]').val(d.mtnp_sn);
								$('#upd_form input[name=mtnp_id]').val(d.mtnp_id);
								$('#upd_form input[name=mtnp_name]').val(d.mtnp_name);
								$('#upd_form [name=mtnp_status] option[value='+d.mtnp_status+']').prop('selected','selected');
			                }
		                },
				...
				$('button.show_photo').on('click', function(){
					...
					$.ajax({
					    ...
					    type: 'post',
				        dataType: 'json',
					    data: {op:"show_photo", mtnp_sn:$(this).data('mtnp_sn')},
					    beforeSend: function(msg) {
		    	        	$("#ajax_loading").show();
		                },
		                complete: function(XMLHttpRequest, textStatus) {
				...
				$('button.add_photo').on('click', function(){
					$('#add_photo_form input[name=mtnp_sn]').val($(this).data('mtnp_sn'));
			            	...
			            	<div class="row">
								<div class="col-md-12">
									...
									<div class="form-group">
										...
										<div class="col-sm-10">
											<select class="form-control" name="mtnp_status">
			            ...
			            <div class="modal-body">
			            	<div class="row">
								<div class="col-md-12">
					        		...
					        		<input type="hidden" name="mtnp_sn">
									<div class="form-group">
										<label class="col-sm-2 control-label">工程<font color="red">*</font></label>
										<div class="col-sm-10">
											<select class="form-control" name="emm_sn">
						...
						<tbody>
							<?php
								foreach ($list as $row) {
									echo '<tr>';
									echo '<td>';
									echo '<button type="button" class="btn btn-primary btn-xs upd" data-mtnp_sn="'.$row['mtnp_sn'].'">修改</button>&nbsp;';
									echo '<button type="button" class="btn btn-danger btn-xs del" data-mtnp_sn="'.$row['mtnp_sn'].'">刪除</button>';
									echo '</td>';
									echo '<td>'.$row['emm_id'].'</td>';
									echo '<td>'.$row['emm_name'].'</td>';
									echo '<td>'.$row['jsuser_name'].'</td>';
									echo '<td>';
									echo '<button type="button" class="btn btn-primary btn-xs show_photo" data-mtnp_sn="'.$row['mtnp_sn'].'">查看照片</button>&nbsp;';
									echo '<button type="button" class="btn btn-primary btn-xs add_photo" data-mtnp_sn="'.$row['mtnp_sn'].'">上傳照片</button>';
									echo '</td>';
									echo '<td>'.date('Y-m-d H:i', $row['mtnp_add_date']).'</td>';
									echo '<td>'.date('Y-m-d H:i', $row['mtnp_mod_date']).'</td>';
									echo '<td>'.$status_mapping[$row['mtnp_status']].'</td>';
									echo '</tr>';
								}
							?>
						</tbody>
