...
if(!empty($op)) {
	...
	switch ($op) {
		case 'export_excel':
			...
			// init excel
			$inputfilename = WT_PATH_ROOT.'/admin/sys/history_online_report.xls';
			if(!file_exists($inputfilename)) exceptions("查無Excel巡檢表");
			$originalexcel = PHPExcel_IOFactory::load($inputfilename);
				
			$search_where = $_SESSION['query']['sys_online_history'];
			$record_list = getHistoryOnline($search_where, 0, 1000000000);
			// printr($record_list);exit;
			// init data
			$add_date = date('Y/m/d H:i:s');
			$sheetname = '所有帳號個別上線時間';
		
			$sheet = $originalexcel->getSheetByName($sheetname);
				
			// 塞值
			$sheet->setCellValue('A1', "所有帳號個別上線時間");
	...
	if(($start = GetParam('start'))) {
		...
		$search_where[] = "jsol_start>={$start_c}";
	}
	if(($end = GetParam('end'))) {
		$end_c = str2time($end);
		$search_where[] = "jsol_start<={$end_c}";
	}
	$search_where = isset($search_where) ? implode(' and ', $search_where) : '';
	$_SESSION['query']['sys_online_history'] = $search_where;
	$search_query_string = isset($search_query_string) ? http_build_query($search_query_string) : '';
	
	// page
	$pg_page = GetParam('pg_page', 1);
	$pg_rows = 20;
	$pg_total = GetParam('pg_total')=='' ? getHistoryOnlineQty($search_where) : GetParam('pg_total');
	$pg_offset = $pg_rows * ($pg_page - 1);
	$pg_pages = $pg_rows == 0 ? 0 : ( (int)(($pg_total + ($pg_rows - 1)) /$pg_rows) );
	
	$history_list = getHistoryOnline($search_where, $pg_offset, $pg_rows);
}
?>
<!DOCTYPE html>
<html lang="en">
		...
		<script type="text/javascript">
			$(document).ready(function() {
				...
				$("#export_excel_btn").click(function(e){
					window.open('./sys_history_online.php?op=export_excel');
				});
			});
		</script>
    </head>
		...
		<div class="container-fluid">
			<div class="row">
				<div class="col-md-12">
    				...
    				<!-- title -->
					<h3 class="text-center wt-block-title">所有帳號個別上線時間</h3>
    				...
    				<div id="search" style="clear:both;">
					    <form autocomplete="off" method="get" action="./sys_history_online.php" id="search_form" class="form-inline alert alert-info" role="form">
					    	<div class="row">
