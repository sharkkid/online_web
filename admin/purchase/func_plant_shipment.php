...
function getUser($where='', $offset=30, $rows=0) {
	...
	$conn = getDB();
	if(empty($where))
		$sql="select * from online_shipment_data where onshda_status>=0 order by onshda_add_date desc, onshda_sn desc limit $offset, $rows";
	else
		$sql="select * from online_shipment_data where onshda_status>=0 and ( $where ) order by onshda_add_date desc, onshda_sn desc limit $offset, $rows";
...
function getUserQty($where='') {
	...
	$conn = getDB();
	if(empty($where))
		$sql="select count(*) from online_shipment_data where onshda_status>=0";
	else
		$sql="select count(*) from online_shipment_data where onshda_status>=0 and ( $where )";
