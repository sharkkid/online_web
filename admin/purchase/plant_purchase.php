...
if(!empty($op)) {
	...
	switch ($op) {
		...
		$onadd_buy_price=GetParam('onadd_buy_price');//採購費用
		...
		if(empty($onadd_location) || empty($onadd_ml_amount)){
			...
			$sql2 = "INSERT INTO onliine_add_data(onadd_part_no,onadd_part_name,onadd_color,onadd_size,onadd_height,onadd_pot_size,onadd_supplier,onadd_planting_date,onadd_quantity,onadd_growing,jsuser_sn,onadd_location,onadd_cur_size,onadd_ml,onadd_add_date,onadd_mod_date,onadd_cycle, onadd_newpot_sn, onadd_sellsize)	
					VALUES('{$onadd_part_no}','{$onadd_part_name}','{$onadd_color}','{$onadd_size}','{$onadd_height}','{$onadd_pot_size}','{$onadd_supplier}','{$onadd_planting_date}','{$onadd_ml_amount}','{$onadd_growing}','{$supplier}','{$onadd_location}','{$onadd_cur_size}','{$onadd_ml}', '{$now}', '{$now}', '{$now}' ,'{$onadd_newpot_sn}', '{$onadd_sellsize}')";
		...
		$jsuser_sn = GetParam('supplier');//編輯人員
		$IsSetCycle=getTargetSize($DEVICE_SYSTEM[$onadd_cur_size],$DEVICE_SYSTEM[$onadd_growing]);//是否有此種植週期
		if(empty($onadd_planting_date)||empty($onadd_quantity)||empty($onadd_growing)||empty($IsSetCycle)){
			if(empty($IsSetCycle))
				$ret_msg = "尚未設定該原始尺寸至預計成長尺寸！";
			else
				$ret_msg = "*為必填！";
				...
				<div class="modal-content">
					<form autocomplete="off" method="post" action="./" id="upd_form" class="form-horizontal" role="form" data-toggle="validator">
						...
						<div class="modal-footer">
							...
							<button type="submit" class="btn btn-primary">確認換盆</button>
						</div>
					</form>
				</div>
			</div>
					...
					<table class="table table-striped table-hover table-condensed tablesorter">
						<thead>
							<tr style="font-size: 1.1em">
								...
								<th style="text-align: center;">預計出貨尺寸</th> <!-- 2020/01/2新增 -->
								...
								echo '<tr>';//產品編號
        							...
        							if(!empty($DEVICE_SYSTEM[$row['onadd_cur_size']])){        								
        								if($row['onadd_cur_size'] == 8){//瓶苗下種
	        								...
	        								echo '<td style="vertical-align: middle;border-right:0.1rem #BEBEBE dashed;text-align: center;">'.$DEVICE_SYSTEM[$row['onadd_cur_size']].'寸</td>';
										}
	        						}
	        						else{
	        						...
	        						//下階段換盆
	        						// printr($row['onadd_growing']);
	        						if(!empty($DEVICE_SYSTEM[$row['onadd_growing']])){
	        							if($DEVICE_SYSTEM[$row['onadd_growing']] == "出貨"){//出貨
											echo '<td style="vertical-align: middle;border-right:0.1rem #BEBEBE dashed;text-align: center;"><font color="#666666">'.$DEVICE_SYSTEM[$row['onadd_growing']].'</font></td>';
        								}
										else{
	        								echo '<td style="vertical-align: middle;border-right:0.1rem #BEBEBE dashed;text-align: center;"><font color="#666666">'.$DEVICE_SYSTEM[$row['onadd_growing']].'寸</font></td>';
										}
	        						}
	        						else{
	        							echo '<td style="vertical-align: middle;border-right:0.1rem #BEBEBE dashed;text-align: center;"></td>';
	        						...
	        						if(!empty($row['onadd_sellsize'])){
        								echo '<td style="vertical-align: middle;border-right:0.1rem #BEBEBE dashed;text-align: center;">'.$DEVICE_SYSTEM[$row['onadd_sellsize']].'寸</td>';
        							}
        							else{
        								echo '<td style="vertical-align: middle;border-right:0.1rem #BEBEBE dashed;text-align: center;"></td>';
        							}
        							...
        							echo '<td style="vertical-align: middle;border-right:0.1rem #BEBEBE dashed;text-align: center;">'.$pre_days.'</td>';//預計成熟日
        							
        							// printr("onadd_cur_size=".$row['onadd_cur_size']);
        							// printr("onadd_sellsize=".$row['onadd_sellsize']);
