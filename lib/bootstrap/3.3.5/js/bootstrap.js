...
+function($) {
	...
	$(document).on('click.bs.button.data-api', '[data-toggle^="button"]',
			function(e) {
				...
				if (!($(e.target).is('input[type="radio"]') || $(e.target)
	...
	Collapse.prototype.getParent = function() {
		return $(this.options.parent)
				.find('[data-toggle="collapse"][data-parent="'
						+ this.options.parent + '"]').each($.proxy(function(i,
								element) {
							var $element = $(element)
...
+function($) {
	...
	Tooltip.prototype.hide = function(callback) {
		...
		function complete() {
			if (that.hoverState != 'in')
				$tip.detach()
			that.$element.removeAttr('aria-describedby').trigger('hidden.bs.'
					+ that.type)
			callback && callback()
		}
	...
	Tooltip.prototype.destroy = function() {
		...
		this.hide(function() {
					that.$element.off('.' + that.type).removeData('bs.'
							+ that.type)
					if (that.$tip) {
						that.$tip.detach()
					}
	...
	ScrollSpy.prototype.process = function() {
		...
		for (i = offsets.length; i--;) {
			activeTarget != targets[i]
					&& scrollTop >= offsets[i]
					&& (offsets[i + 1] === undefined || scrollTop < offsets[i
							+ 1]) && this.activate(targets[i])
		}
	}
